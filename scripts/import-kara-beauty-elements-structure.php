...
$sourceProducts = collectBeautyElementsKaraProducts();
$groups = groupSourceProducts($sourceProducts);

if ($dryRun) {
    echo "KARA Beauty Elements dry run.\n";
...
$summary = DB::transaction(function () use ($groups): array {
    ...
    $brand = BrandCatalogueBrand::query()->firstOrCreate(
        ...
        [
            'slug' => scopedSlug(BrandCatalogueBrand::query()->where('brand_catalogue_id', $catalogue->id), 'KARA'),
            'is_active' => true,
            'sort_order' => 95,
        ],
    );
    ...
    $brand->fill([
        'note' => mergeNote($brand->note, 'Reference structure imported from Beauty Elements KARA catalogue. Confirm shop stock before publishing retail products.'),
        'url' => $brand->url ?: karaBrandUrl(),
        'is_active' => true,
    ])->save();
    ...
    foreach ($groups as $group) {
        ...
        $line = BrandCatalogueLine::query()->firstOrCreate(
            ...
            [
                ...
                'is_default' => $group['line'] === 'KARA',
                'is_active' => true,
            ],
        );

        $line->fill([
            'note' => mergeNote($line->note, 'Beauty Elements KARA source line.'),
        ...
        $productType->fill([
            'brand_catalogue_brand_id' => $brand->id,
            'note' => mergeNote($productType->note, 'Operational product type structured from Beauty Elements KARA product data.'),
...
echo "KARA Beauty Elements structure imported.\n";
foreach ($summary as $key => $value) {
    echo "{$key}: {$value}\n";
}

function karaBrandUrl(): string
{
    return 'http://beautyelements.co/brand/brand-kara/';
}

/**
 * @return Collection<int, array<string, mixed>>
 */
function collectBeautyElementsKaraProducts(): Collection
{
    $links = parseBeautyElementsBrandGrid(karaBrandUrl());
    $products = collect();
    $seen = [];
...
function parseBeautyElementsProductPage(array $link): ?array
{
    ...
    $pageText = productPageText($xpath);
    $description = productTextSection($xpath, 'PRODUCT DESCRIPTION');
    $additionalInfo = productTextSection($xpath, 'ADDITIONAL INFORMATION') ?: productAvailableColoursText($pageText);
    $sourceProductStyle = sourceProductStyle($pageText);
    $sourceMaterial = sourceMaterial($pageText);
    $lengths = extractLengthOptions($sourceTitle.' '.$description.' '.$pageText);
    $colours = extractColourOptions($additionalInfo);
    $styleName = styleName($rawStyleName, $lengths);
    $productType = productTypeName($link['classes'], $styleName, $rawStyleName, $sourceProductStyle);
    $material = materialName($link['classes'], $sourceMaterial);
    $options = [];
    ...
    foreach ($imageUrls as $index => $imageUrl) {
        CatalogueImage::query()->updateOrCreate(
            ...
            [
                ...
                'notes' => 'Beauty Elements KARA reference image for '.$style->name.'.',
...
function syncStyleVariantsAndSkus(BrandCatalogueStyle $style, array $group, Collection $products): array
{
    ...
    foreach ($expandedSkus as $index => $sourceSku) {
        ...
        $note = 'Beauty Elements KARA source: '.$source['source_title'].'.';
...
function styleNote(Collection $products): string
{
    ...
    return 'Reference style imported from Beauty Elements KARA products: '.$titles.'. Confirm shop stock before publishing retail products.'.$pending;
}
...
function styleName(string $rawStyle, array $lengths): string
{
    ...
    $style = preg_replace('/\bHB\s*-\s*/iu', 'HB ', $style) ?? $style;
    $style = preg_replace('/\bSUPER\s+DELUX\s*-\s*/iu', 'Super Delux ', $style) ?? $style;
 ...
 */
function productTypeName(array $tags, string $styleName, string $rawStyleName, string $sourceProductStyle): string
{
    $haystack = Str::lower($styleName.' '.$rawStyleName.' '.$sourceProductStyle);

    if (hasTag($tags, 'drawstring') && Str::contains($haystack, 'puff')) {
        return 'Hair Puffs / Hair Pieces';
    }

    if (Str::contains($haystack, 'lace wig')) {
        return 'Lace Wigs';
    }

    if (Str::contains($haystack, 'full wig')) {
        return 'Wigs';
    }
 ...
 * @param array<int, string> $tags
 */
function materialName(array $tags, string $sourceMaterial): string
{
    $sourceMaterial = Str::lower($sourceMaterial);

    if (str_contains($sourceMaterial, 'brazilian')) {
        return 'Brazilian Human Hair';
    }

    if (str_contains($sourceMaterial, 'human hair blend')) {
        return 'Human Hair Blend';
    }

    if (str_contains($sourceMaterial, 'human hair')) {
        return 'Human Hair';
    }

    if (str_contains($sourceMaterial, 'synthetic')) {
...
function productPageText(DOMXPath $xpath): string
{
    $blocks = [];
    foreach ($xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' avia_textblock ')]") as $node) {
        $text = cleanText($node->textContent);
    ...
    return cleanSpaces(implode(' ', $blocks));
}

function productAvailableColoursText(string $pageText): string
{
    if (preg_match('/(?:Available\s+Colors?|Color\s+Chart)\s*[-:\x{2013}\x{2014}]\s*(.+?)(?:CHECK OUR PRODUCTS BY BRAND|PRODUCT STYLE:|HAIR MATERIAL:|$)/iu', $pageText, $match)) {
...
function sourceProductStyle(string $pageText): string
{
    if (preg_match('/PRODUCT\s+STYLE:\s*(.+?)(?:HAIR\s+MATERIAL:|Available\s+Colors?|Color\s+Chart|$)/iu', $pageText, $match)) {
        return cleanSpaces($match[1]);
    }
...
function sourceMaterial(string $pageText): string
{
    if (preg_match('/HAIR\s+MATERIAL:\s*(.+?)(?:PRODUCT\s+STYLE:|Available\s+Colors?|Color\s+Chart|$)/iu', $pageText, $match)) {
        return cleanSpaces($match[1]);
    }
...
function titleStyle(string $value): string
{
    ...
    return cleanSpaces(implode(' ', array_map(function (string $word): string {
        ...
        if (preg_match('/^(?:\d+x|3d|hh|hb|ht|tif|lb\d+|lfp\d+|brl\d+|kcl\d+|plf\d+|dp|de|f\d+|p\d+|tp\d+)$/i', $trimmed)) {
