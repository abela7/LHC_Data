...
$sourceProducts = collectBeautyElementsKaliProducts();
$groups = groupSourceProducts($sourceProducts);

if ($dryRun) {
    echo "Kali Beauty Elements dry run.\n";
...
$summary = DB::transaction(function () use ($groups): array {
    ...
    $brand = BrandCatalogueBrand::query()->firstOrCreate(
        ...
        [
            'slug' => scopedSlug(BrandCatalogueBrand::query()->where('brand_catalogue_id', $catalogue->id), 'Kali'),
            'is_active' => true,
            'sort_order' => 90,
        ],
    );
    ...
    $brand->fill([
        'note' => mergeNote($brand->note, 'Reference structure imported from Beauty Elements Kali catalogue. Confirm shop stock before publishing retail products.'),
        'url' => $brand->url ?: kaliBrandUrl(),
        'is_active' => true,
    ])->save();
    ...
    foreach ($groups as $group) {
        ...
        $line = BrandCatalogueLine::query()->firstOrCreate(
            ...
            [
                ...
                'is_default' => $group['line'] === 'Kali',
                'is_active' => true,
            ],
        );

        $line->fill([
            'note' => mergeNote($line->note, 'Beauty Elements Kali source line.'),
        ...
        $productType->fill([
            'brand_catalogue_brand_id' => $brand->id,
            'note' => mergeNote($productType->note, 'Operational product type structured from Beauty Elements Kali product data.'),
...
echo "Kali Beauty Elements structure imported.\n";
foreach ($summary as $key => $value) {
    echo "{$key}: {$value}\n";
}

function kaliBrandUrl(): string
{
    return 'http://beautyelements.co/brand/kali/';
}

/**
 * @return Collection<int, array<string, mixed>>
 */
function collectBeautyElementsKaliProducts(): Collection
{
    $links = parseBeautyElementsBrandGrid(kaliBrandUrl());
    $products = collect();
    $seen = [];
    ...
    foreach ($imageUrls as $index => $imageUrl) {
        CatalogueImage::query()->updateOrCreate(
            ...
            [
                ...
                'notes' => 'Beauty Elements Kali reference image for '.$style->name.'.',
...
function syncStyleVariantsAndSkus(BrandCatalogueStyle $style, array $group, Collection $products): array
{
    ...
    foreach ($expandedSkus as $index => $sourceSku) {
        ...
        $note = 'Beauty Elements Kali source: '.$source['source_title'].'.';
...
function styleNote(Collection $products): string
{
    ...
    return 'Reference style imported from Beauty Elements Kali products: '.$titles.'. Confirm shop stock before publishing retail products.'.$pending;
}
