...
$sourceProducts = collectBeautyElementsKaliEssentialProducts();
$groups = groupSourceProducts($sourceProducts);

if ($dryRun) {
    echo "Kali Essential Beauty Elements dry run.\n";
...
$summary = DB::transaction(function () use ($groups): array {
    ...
    $brand = BrandCatalogueBrand::query()->firstOrCreate(
        ...
        [
            'slug' => scopedSlug(BrandCatalogueBrand::query()->where('brand_catalogue_id', $catalogue->id), 'Kali Essential'),
            'is_active' => true,
            'sort_order' => 100,
        ],
    );
    ...
    $brand->fill([
        'note' => mergeNote($brand->note, 'Reference structure imported from Beauty Elements Kali Essential catalogue. Confirm shop stock before publishing retail products.'),
        'url' => $brand->url ?: kaliEssentialBrandUrl(),
        'is_active' => true,
    ])->save();
    ...
    foreach ($groups as $group) {
        ...
        $line = BrandCatalogueLine::query()->firstOrCreate(
            ...
            [
                ...
                'is_default' => $group['line'] === 'Kali Essential',
                'is_active' => true,
            ],
        );

        $line->fill([
            'note' => mergeNote($line->note, 'Beauty Elements Kali Essential source line.'),
        ...
        $productType->fill([
            'brand_catalogue_brand_id' => $brand->id,
            'note' => mergeNote($productType->note, 'Operational product type structured from Beauty Elements Kali Essential product data.'),
...
echo "Kali Essential Beauty Elements structure imported.\n";
foreach ($summary as $key => $value) {
    echo "{$key}: {$value}\n";
}

function kaliEssentialBrandUrl(): string
{
    return 'http://beautyelements.co/brand/brand-kali-essential/';
}

/**
 * @return Collection<int, array<string, mixed>>
 */
function collectBeautyElementsKaliEssentialProducts(): Collection
{
    $links = parseBeautyElementsBrandGrid(kaliEssentialBrandUrl());
    $products = collect();
    $seen = [];
...
function parseBeautyElementsBrandGrid(string $url): array
{
    ...
    foreach ($xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' grid-entry ')]") as $entry) {
        ...
        if (Str::upper($excerpt) !== 'KALI ESSENTIAL') {
            continue;
        }
...
function parseBeautyElementsProductPage(array $link): ?array
{
    ...
    return [
        'line' => $lineName,
        'line_url' => kaliEssentialBrandUrl(),
    ...
    foreach ($imageUrls as $index => $imageUrl) {
        CatalogueImage::query()->updateOrCreate(
            ...
            [
                ...
                'notes' => 'Beauty Elements Kali Essential reference image for '.$style->name.'.',
...
function syncStyleVariantsAndSkus(BrandCatalogueStyle $style, array $group, Collection $products): array
{
    ...
    foreach ($expandedSkus as $index => $sourceSku) {
        ...
        $note = 'Beauty Elements Kali Essential source: '.$source['source_title'].'.';
...
function styleNote(Collection $products): string
{
    ...
    return 'Reference style imported from Beauty Elements Kali Essential products: '.$titles.'. Confirm shop stock before publishing retail products.'.$pending;
}
...
function splitSourceTitle(string $sourceTitle): array
{
    ...
    return ['Kali Essential', cleanSpaces($title)];
}
...
{
    return 'Kali Essential '.$styleName;
}
