...
@section('title', $material->name . ' - View')

@section('content')
    @include('brand-catalogue._page-label', ['label' => 'Material View', 'context' => $material->name])
    ...
    <div class="bcv-grid">
        @foreach ($material->styles as $style)
            <a href="{{ route('brand-catalogue.view.style', [$catalogue, $brand, $line, $productType, $material, $style]) }}" class="bcv-card">
                <div class="bcv-card-top"><h3 class="bcv-card-name">{{ $style->name }}</h3></div>
